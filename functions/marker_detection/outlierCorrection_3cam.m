function [utvid] = outlierCorrection_3cam(utvid)
...
for i=1:nrIter
    ...
    PCAreconEr = sqrt(sum(( ...
        ...
        [utvid.Tracking.Kal.meas(1:end/2,utvid.Tracking.n), ...
        utvid.Tracking.Kal.meas((end/2+1:end),utvid.Tracking.n)] ...
        ).^2, 2));
...
if size(C{maxInd},1)~= 0
    ptsCorr = utvid.Tracking.Kal.meas(ind_sub,utvid.Tracking.n);
...
%Determine outliers
utvid.Tracking.outliers{1} = C{maxInd}(C{maxInd}<=N);
