function [PCAcoords,PCAmodel,Xest,Kal] = PCAExpansion(FrameL,FrameR,FrameM,n,PCAcoords,Kal,Xest,PCAmodel,Pstruct,lim,utvid)
Kal.meas(:,n)
Kal.Xpred(:,n)
Xest.x1(:,:,n)
...
if utvid.settings.nrOrMar ~= 0
    ...
    switch choice
        ...
        case 'No'
            ...
            correctPoints(imnL,utvid.settings.nrOrMar,Xest_or.x3(:,:,n),'orientation')
...
hold on
imshow(imnL,[]); hold on,
plot(Xest.x1(:,1,n),Xest.x1(:,2,n),'*r');
...
figure; set(gcf, 'Position', get(0,'Screensize'));
imshow(imnR,[]);  hold on
plot(Xest.x2(:,1,n),Xest.x2(:,2,n),'*r');
choice = questdlg('Markers located correct', 'User input', ...
    'Yes','No','No');
switch choice
    case 'Yes'
...
figure; set(gcf, 'Position', get(0,'Screensize'));
imshow(imnM,[]);  hold on
plot(Xest.x3(:,1,n),Xest.x3(:,2,n),'*r');
choice = questdlg('Markers located correct', 'User input', ...
    'Yes','No','No');
switch choice
    case 'Yes'
...
% if Dn > lim
    ...
    PCAcoords = [PCAcoords,compVec]; PCAsize = size(PCAcoords,2)
    utvid = getPCAmodel(utvid);
    
    % Rotate PCA model, when using orientation markers, volgens mij gaat
    % dit nog niet goed met orientatie markers.
    if utvid.settings.nrOrMar ~= 0
