function X = getAllRep(X, iter, vec, unc, Pstruct)
% In the system, three different 'spaces' are considered: the 2D space, the
% 3D space and the PCA space. Conversion from one space to the others is
% possible. This function calculates the state of all these spaces given
% the representation in a single space. This is then stored in the variable
% X, which keeps track of these states in time.
% 
...
%                      - if 3D:     3Nx3N matrix
%                      - if PCA:    MxM matrix
%           rep:    representation: either '2D', '3D' or 'PCA'
%           PCAmodel:       PCA model
%           Pstruct:        structure containing Pext, and calibration
%                           matrices
...
%     %defines PCA state
%     if settings.nrCam==3
%         [X.PC(:,iter), X.C_PC(:,:,iter)] = twoDtoPCA_3cam(vec, unc, PCAmodel, Pstruct);
%     else
%         [X.PC(:,iter), X.C_PC(:,:,iter)] = twoDtoPCA(vec, unc, PCAmodel, Pstruct);
%     end
% 
...
%     
    ...
    %defines PCA state
%     [X.PC(:,iter), X.C_PC(:,:,iter)] = threeDtoPCA(vec, unc, PCAmodel);
    
% elseif strcmp(rep, 'PCA')
%     
%     %defines 2D state
%     if settings.nrCam==3;
%         [X.x1(:,:,iter), X.x2(:,:,iter), X.x3(:,:,iter), X.C_x1(:,:,:,iter), X.C_x2(:,:,:,iter), X.C_x3(:,:,:,iter)] = ...
%             PCAto2D_3cam(vec, unc, PCAmodel, Pstruct);
%     else
%         [X.x1(:,:,iter), X.x2(:,:,iter), X.C_x1(:,:,:,iter), X.C_x2(:,:,:,iter)] = ...
%             PCAto2D(vec, unc, PCAmodel, Pstruct);
%     end
%     
%     %defines 3D state
%     [X.X(:,:,iter), X.C_X(:,:,:,iter)] = PCAto3D(vec, unc, PCAmodel);
%     
%     %defines PCA state
%     X.PC(:,iter) = vec;
%     X.C_PC(:,:,iter) = unc;
