...
utvid.Tracking.Kal.Xpred(:,utvid.Tracking.n+1)
...
if size(utvid.pca.PCAcoords,2) > 4*utvid.settings.PCs
    ...
    if utvid.settings.nrOrMar ~= 0
        ...
        [utvid,utvid.Tracking.rt_coor(:,utvid.Tracking.n),c] = outlierCorrectionMMSE(utvid,utvid.Tracking.Kal.Xest(1:end/2,utvid.Tracking.n));
        coor_back = utvid.Tracking.rt_coor(:,utvid.Tracking.n);
        ...
        if ~isempty(c)
            ...
            for cc = 1:length(c)
                ...
                % zet predictie naar de gecorrigeerde versie
%                 utvid.Tracking.Kal.Xpred(c(cc),utvid.Tracking.n+1) = coor_back(c(cc));
%                 utvid.Tracking.Kal.Xpred(utvid.settings.nrMarkers+c(cc),utvid.Tracking.n+1) = coor_back(c(cc)+utvid.settings.nrMarkers);
%                 utvid.Tracking.Kal.Xpred(2*utvid.settings.nrMarkers+c(cc),utvid.Tracking.n+1) = coor_back(c(cc)+utvid.settings.nrMarkers*2);
                ...
                % zet estimate naar de gecorrigeerde versie
                utvid.Tracking.Kal.Xest(c(cc),utvid.Tracking.n) = coor_back(c(cc));
                utvid.Tracking.Kal.Xest(utvid.settings.nrMarkers+c(cc),utvid.Tracking.n) = coor_back(c(cc)+utvid.settings.nrMarkers);
                utvid.Tracking.Kal.Xest(2*utvid.settings.nrMarkers+c(cc),utvid.Tracking.n) = coor_back(c(cc)+utvid.settings.nrMarkers*2);
...
utvid.Tracking.Xpred = getAllRep( utvid.Tracking.Xpred, utvid.Tracking.n+1, utvid.Tracking.Kal.Xpred(1:end/2,utvid.Tracking.n), ...
    utvid.Tracking.Kal.Cpred(1:end/2,1:end/2,utvid.Tracking.n+1), utvid.Pstruct);
...
if utvid.Tracking.plotting == 1
    ...
    if utvid.pca.outlier == 1;
        ...
        plot(test.Kal.meas(1:utvid.settings.nrMarkers), ...
            test.Kal.meas(utvid.settings.nrMarkers*3+1:...
            utvid.settings.nrMarkers*4),'g.');
    end
    %
...
if ~isempty(c)
    utvid.Tracking.Xpred.X(:,:,utvid.Tracking.n:utvid.Tracking.n+1)
end

utvid.Tracking.Xpred = getAllRep( utvid.Tracking.Xpred, utvid.Tracking.n+1,...
    utvid.Tracking.Kal.Xpred(1:end/2,utvid.Tracking.n+1), ...
    utvid.Tracking.Kal.Cpred(1:end/2,1:end/2,utvid.Tracking.n+1), utvid.Pstruct);
