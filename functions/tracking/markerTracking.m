...
if utvid.settings.initTracking
    ...
    if utvid.Tracking.plotting == 1
        if ishandle(handles.hax{1,1}), for i = 1:3, cla(handles.hax{1,i}), end, end
...
%% get text box values
    function gettextvalues(trackingFigure,handles)
        ...
        if isempty(utvid.Tracking.sigMeas);    utvid.Tracking.sigMeas = 1;
            set(handles.h{7},'string','1');     end
        if isempty(utvid.Tracking.sigVx);      utvid.Tracking.sigVx = 10;
            set(handles.h{8},'string','10');     end
        if isempty(utvid.Tracking.sigVy);      utvid.Tracking.sigVy = 10;
            set(handles.h{9},'string','10');     end
        if isempty(utvid.Tracking.sigVz);      utvid.Tracking.sigVz = 10;
            set(handles.h{10},'string','10');    end
